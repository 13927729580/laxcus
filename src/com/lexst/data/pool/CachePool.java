...
public class CachePool extends LocalPool {
	...
	private LinkedList<EntityIdentity> deletes = new LinkedList<EntityIdentity>();
	 ...
	 * @param data
	 */
	public boolean setCacheReflex(String db, String table, long chunkid, byte[] data) {
		int ret = Install.setCacheReflex(db.getBytes(), table.getBytes(), chunkid, data);
		return ret == 0;
	}
	 ...
	 * @return
	 */
	public boolean deleteCacheReflex(String db, String table, long chunkid) {
		int ret = Install.deleteCacheReflex(db.getBytes(), table.getBytes(), chunkid);
		return ret == 0;
	}
	...
	public boolean delete(String db, String table, long chunkid) {
		lock.lock();
		try {
			return deletes.add(new EntityIdentity(db, table, chunkid));
		} catch (Throwable exp) {
			Logger.error(exp);
		} finally {
			lock.unlock();
	...
	private SocketHost[] choose(SiteHost[] hosts, int limit) {
		...
		for(SiteHost host : hosts) {
			...
			SocketHost udp = host.getPacketHost();
	...
	/**
	 * save cache data, only prime site
	 * @param space
	 * @param cacheid
	 * @param data
	 * @return
	 */
	public boolean update(Space space, long cacheid, byte[] data) {
		Table table = Launcher.getInstance().findTable(space);
		if (table == null) {
			Logger.error("CachePool.update, cannot find '%s'", space);
			return true;
		} else if (table.getCopy() < 2) {
			Logger.info("CachePool.update, chunk number is:%d, not backup!", table.getCopy());
			return false;
		}
		
		Logger.debug("CachePool.update, space:'%s', chunkid:%x, data len:%d", space, cacheid, (data == null ? -1 : data.length));
		...
		if(element == null) {
			element = new EntityElement(space.getSchema(), space.getTable(), cacheid); // schema_name, table_name, cacheid);
			// 去HOME节点找DATA节点地址
			SiteHost home = Launcher.getInstance().getHubSite();
			...
			SiteHost[] sites = null;
			try {
				sites = client.findDataSite(space.getSchema(), space.getTable());
	 ...
	 * @return
	 */
	public boolean update(String schema, String table, long cacheid, byte[] data) {
		Space space = new Space(schema, table);
		return update(space, cacheid, data);
	}
	...
	public boolean resolve(byte[] data) {
		...
		for(int i = 0; i < size; i++) {
			...
			for(int j = 0; j < len; j++) {
				...
				try {
					SocketHost host = new SocketHost(SocketHost.UDP, InetAddress.getByName(ip), Integer.parseInt(port));
					entity.addHost(host);
	...
	public byte[] build() {
		...
		for(long id : mapEntity.keySet()) {
			...
			for(SocketHost host : element.listHosts()) {
				String ip = XML.element("ip", host.getSpecifyAddress());
