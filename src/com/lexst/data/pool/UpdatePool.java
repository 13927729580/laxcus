...
/**
 *
 * 接收来自HOME的通知，更新BUILD节点上的CHUNK数据。更新完成后，通过HOME通知对应的"从节点"
 * 只限主节点操作
 */
public class UpdatePool extends LocalPool {
	...
	private boolean execute(HomeClient homeClient, Update object) {
		...
		Map<SiteHost, List<ChunkStatus>> mapInfo = new HashMap<SiteHost, List<ChunkStatus>>();
		for (SiteHost host : object.list()) {
			...
			try {
				ChunkStatus[] all = buildClient.findChunkInfo(space);
				if (all != null) {
					ArrayList<ChunkStatus> array = new ArrayList<ChunkStatus>();
					for (ChunkStatus info : all) {
						array.add(info);
					}
		...
		for (SiteHost host : mapInfo.keySet()) {
			List<ChunkStatus> array = mapInfo.get(host);
	...
	private boolean download(int num, BuildClient client, Space space, ChunkStatus info) {
		boolean success = false;
		for (int i = 0; i < num; i++) {
			if (i > 0) {
				this.complete(client);
	...
	private boolean download(BuildClient client, Space space, ChunkStatus info) {
		byte[] db = space.getSchema().getBytes();
		byte[] table = space.getTable().getBytes();
		...
		// build chunk name
		long chunkid = info.getId();
		String filename = buildChunkFile(new String(path), chunkid);
		// download chunk
