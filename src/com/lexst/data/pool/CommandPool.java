...
public class CommandPool extends LocalPool {
	 ...
	 * @return
	 */
	public boolean deflate(Space space, short columnId) {
		...
		for (CommandType task : array.list()) {
			if (task.type == CommandType.REBUILD && task.space.equals(space)) {
				return false;
			}
		}
		boolean success = array.add(new CommandType(CommandType.REBUILD, space, columnId));
		if (success) {
			queue.add(space);
		}
		if(success) wakeup();
	...
	private void execute() {
		while(!array.isEmpty()) {
			...
			case CommandType.REBUILD:
				rebuild(task.space, task.columnId);
	...
	/**
	 * 重构表，指定表的键(键默认是0时，重构键将使用原主键)
	 * @param space
	 * @param columnId
	 * @return
	 */
	private boolean rebuild(Space space, short columnId) {
		byte[] schema = space.getSchema().getBytes();
		byte[] table = space.getTable().getBytes();
		
		long time = System.currentTimeMillis();
		Logger.info("CommandPool.rebuild, space '%s'", space);
		// 调用JNI接口实现重构(删除过期数据,重新压缩数据并且存储)
		byte[] bytes = Install.rebuild(schema, table, columnId);
		Logger.info("CommandPool.rebuild, '%s' usedtime %d, byte size %d",
		...
		Logger.info("CommandPool.rebuild, old chunkid size:%d, new chunkid size:%d",
		...
		if(client == null) {
			Logger.error("CommandPool.rebuild, cannot connect to home site %s", home);
			return false;
		}
		...
		Logger.note(success, "CommandPool.rebuild, upgrade '%s' to home site %s", space, home);
		return success;
	}
