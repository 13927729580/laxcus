...
public class ChunkPool extends LocalPool {
	 ...
	 * @return
	 */
	public boolean setChunkReflex(String schema, String table, long chunkid, byte[] data) {
		// only slave site
		DataSite site = Launcher.getInstance().getLocal();
		if (!site.isSlave()) {
			Logger.error("ChunkPool.setChunkReflex, current site rank is %d", site.getRank());
			return false;
		}
		// write data to local disk
		int ret = Install.setChunkReflex(schema.getBytes(), table.getBytes(), chunkid, data);
		return ret == 0;
	}
	 ...
	 * @param data
	 */
	public boolean update(String schema, String table, long chunkid, byte[] data) {
		...
		if(element == null) {
			element = new EntityElement(schema, table, chunkid);
			...
			SiteHost[] sites = null;
			try {
				sites = client.findDataSite(schema, table, chunkid);
			...
			if(sites == null) {
				Logger.error("ChunkPool.update, cannot find data site '%s-%s %X'", schema, table, chunkid);
				return false;
			}
			...
			for(SiteHost site : sites) {
				...
				element.addHost(site.getPacketHost());
			}
	 ...
	 * @return
	 */
	public boolean update(Space space, long chunkid, byte[] data) {
		return this.update(space.getSchema(), space.getTable(), chunkid, data);
	}
