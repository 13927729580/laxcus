...
public class DataStreamInvoker implements StreamInvoker {
	...
	public void invoke(Stream request, OutputStream resp)
		...
		case Request.SQL:	//可以执行
			...
			case Request.SQL_CONDUCT:
				this.conduct(request, resp); break;
	 ...
	 * @throws IOException
	 */
	private void conduct(Stream request, OutputStream resp) throws IOException {
		Logger.debug("DataStreamInvoker.conduct, site from %s", request.getRemote());
		
		byte[] data = request.readContent();
		Apply apply = this.resolve(data);
		FromPhase phase = (FromPhase)apply.getParameters()[0];
		ConductPool.getInstance().conduct(phase, resp);
	}
