...
public class SQLWindow extends JFrame implements ChangeListener, ActionListener {
	...
	private ResourceLoader imageLoader = new ResourceLoader("conf/terminal/image/window/");
	...
	public void flicker() {
		...
		for (int i = 0; i < images.length; i++) {
			icons[i] = imageLoader.findImage(images[i]);
		}
		...
		ImageIcon icon = imageLoader.findImage("color_green.png");
		lblTop.setIcon(icon);
	}
	...
	public void disconnect() {		
		ImageIcon icon = imageLoader.findImage("color_gray.png");
		lblTop.setIcon(icon);
	...
	public void setDBA() {
		ImageIcon icon = imageLoader.findImage("dba_16.png");
	...
	public void setUser() {
		ImageIcon icon = imageLoader.findImage("client_16.png");
		lblRank.setIconTextGap(8);
	...
	public void setUnknown() {
		ImageIcon icon = imageLoader.findImage("unclient_16.png");
		lblRank.setIconTextGap(8);
	...
	private boolean login(boolean init) {
		...
		SiteHost host = TouchPool.getInstance().getRemote();
		setTop(String.format("top site: %s:%d", host.getSpecifyAddress(), host.getTCPort()));
		
		menu.itemLogin.setEnabled(false);
		// set user admin
		if (TouchPool.getInstance().isDBA()) {
			setDBA();
		} else if (TouchPool.getInstance().isUser()) {
			setUser();
		} else {
			this.setUnknown();
		}
		// refresh data
		left.review(TouchPool.getInstance().listSchema(), TouchPool.getInstance().listTable());
	...
	private void logout() {
		...
		TouchPool.getInstance().logout(local);
		left.clear();
		setUnknown();
		disconnect();
	 ...
	 */
	public void refresh() {
		TouchPool.getInstance().resetUser();
		LiveSite local = Launcher.getInstance().getLocal();
			
		// check identity
		TouchPool.getInstance().checkIdentified(null, local);
		int ret1 = TouchPool.getInstance().getPermits(null, local);
		int ret2 = TouchPool.getInstance().getSchemas(null, local);
		int ret3 = TouchPool.getInstance().getTables(null, local);
		if (ret1 >= 0 && ret2 >= 0 && ret3 >= 0) {
			// refresh table configure
			left.review(TouchPool.getInstance().listSchema(), TouchPool.getInstance().listTable());
		} else {
			SiteHost host = TouchPool.getInstance().getRemote();
			String msg = String.format("Cannot connect to %s:%d", host.getSpecifyAddress(), host.getTCPort());
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == menu.itemLogin) {
			...
			this.exit();
		} else if(e.getSource() == menu.itemHelp) {
			SQLHelp help = new SQLHelp(this, true);
			help.showDialog();
		} else if(e.getSource() == menu.itemAbout) {
			SQLAbout about = new SQLAbout(this, true);
			about.showDialog();
...
//		}
	...
	private boolean loadLog() {
		ResourceLoader ext = new ResourceLoader();
		byte[] data = ext.findStream("conf/terminal/xml/window/log.xml");
	...
	public boolean showWindow() {
		...
		this.setTitle(title);
		ImageIcon icon = imageLoader.findImage("logo.png");
	...
	private JPanel initBottom() {
		...
		for (int i = 0; i < subs.length; i++) {
			ImageIcon icon = imageLoader.findImage(images[i]);
	...
	private void updateUI(String clsName) {
		...
		SwingUtilities.updateComponentTreeUI(super.getRootPane());
...
//		SwingUtilities.updateComponentTreeUI(menuBar);
	...
	private boolean loadTrayIcon() {
		ImageIcon icon = imageLoader.findImage("logo.png", 16, 16);
