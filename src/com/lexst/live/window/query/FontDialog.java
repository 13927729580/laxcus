...
public class FontDialog extends JDialog {
	 ...
	 * 返回字体变量
	 * */
	private static Font returnFont;
	...
	/**
	 * 以防止事件重复调用或不必要的更改,定义两个boolean变量分别 为:fontNameList和fontSizeList判断
	 * 等于true则循环调用,false则不
	 * */
	...
	public FontDialog(JFrame frame, boolean modal) {
		this(frame, modal, null);
	}

	public FontDialog(JFrame frame, boolean modal, Font font) {
		super(frame, modal);
	...
	private void initialize() {
		...
		this.setTitle("Lexst SQL Font");

		this.listName.setSelectedValue(txtName.getText(), true);
	...
	public static Font showDialog(JFrame jframe, boolean modal) {
		return showDialog(jframe, modal, null);
	}

	public static Font showDialog(JFrame jframe, boolean modal, Font font) {
		JDialog dialog = new FontDialog(jframe, modal, font);
	...
	private JTextField getFontNameText() {
		if (txtName == null) {
			...
			txtName.setText(defaultFont.getName());
				...
				public void keyTyped(KeyEvent e) {
					...
					if ("".equals(txtName.getSelectedText())
						...
						newText = oldText.substring(0,
								txtName.getSelectionStart())
								+ e.getKeyChar()
								+ oldText.substring(txtName.getSelectionEnd());
					}
					// System.out.println("fontName:" + newText);
					nameJuge = false;
					listName.setSelectedValue(getLateIndex(listName, newText), true);
					nameJuge = true;
				}
			});
		}
	...
	private JList getFontNameList() {
		if (listName == null) {
			listName = new JList(GraphicsEnvironment.getLocalGraphicsEnvironment().getAvailableFontFamilyNames());
			listName.addListSelectionListener(new javax.swing.event.ListSelectionListener() {
				public void valueChanged(javax.swing.event.ListSelectionEvent e) {
					if (nameJuge) {
						String name = listName.getSelectedValue().toString();
						if (name != null && name.trim().length() > 0) {
							txtName.setText(name);
						}
					}
					lblFont.setFont(new Font(listName.getSelectedValue().toString(), 
						lblFont.getFont().getStyle(), lblFont.getFont().getSize()));
				}
			});
		}
		return listName;
	...
	private JList getFontSizeList() {
		if (listSize == null) {
			...
			listSize.addListSelectionListener(new javax.swing.event.ListSelectionListener() {
				public void valueChanged(javax.swing.event.ListSelectionEvent e) {
					if (sizeJuge) {
						String size = listSize.getSelectedValue().toString();
						if (size != null && size.trim().length() > 0) {
							txtSize.setText(size);
						}
					}
					lblFont.setFont(new Font(lblFont.getFont().getFontName(),
							lblFont.getFont().getStyle(), Integer
									.parseInt(listSize.getSelectedValue()
											.toString())));
				}
			});
		}
