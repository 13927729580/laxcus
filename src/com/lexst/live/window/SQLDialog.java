...
public class SQLDialog extends JDialog implements ActionListener {
	...
	private boolean login() {
		...
		String strIP = txtIP.getText().trim();
		...
		SiteHost remote = null;
		try {
			Address address = new Address(strIP);
			int port = Integer.parseInt(strPort);
			remote = new SiteHost(address, port, port);
		} catch (UnknownHostException e) {
			showWarning("server address error", "argument error");
			txtIP.requestFocus();
			return false;
		}
		...
		// 注册到TOP节点
		boolean success = TouchPool.getInstance().login(remote, local);
		if (!success) {
			String s = String.format("login to \'%s:%s\' failed, check address or account, please!", strIP, strPort);
