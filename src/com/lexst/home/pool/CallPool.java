...
/**
 * 
 * HOME节点上，监管各CALL节点的资源配置池。<br>
 *
 */
public class CallPool extends ControlPool {
	...
	public boolean createSpace(Table table) {
		...
		try {
			for (SiteHost socket : mapSite.keySet()) {
				...
				if (site.countSpaces() < count) {
					count = site.countSpaces();
					host = socket;
				}
			}
		} catch (Throwable exp) {
		...
		SocketHost socket = site.getHost().getStreamHost();
	...
	public boolean deleteSpace(Space space) {
		...
		for (SiteHost site : sites) {
			CallClient client = apply(site.getStreamHost());
			try {
				if (client != null) {
	...
	public boolean add(Site object) {
		...
		Logger.info("CallPool.add, call site %s, naming size:%d, space size:%d",
				host, site.listAllNaming().size(), site.getSpaces().size());
