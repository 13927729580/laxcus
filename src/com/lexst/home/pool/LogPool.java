...
/**
 * 
 * HOME节点上，监管各LOG节点的配置池。<br>
 *
 */
public class LogPool extends ControlPool {
	...
	public boolean add(Site obj) {
		if (obj == null || obj.getFamily() != Site.LOG_SITE) {
	...
	public SiteHost find(int type) {
		...
		try {
			...
			return new SiteHost(logSite.getHost().getAddress(), node.getPort(), node.getPort());
		} catch (Throwable exp) {
			Logger.error(exp);
		} finally {
			super.unlockSingle();
