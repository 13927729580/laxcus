...
/**
 * 
 * HOME节点上，监管各DATA节点的资源管理池。<br>
 *
 */
public class DataPool extends ControlPool {
	 ...
	 * @return
	 */
	private List<SiteHost> findHosts(Space space, Address[] addresses, boolean prime) {
			...
			if (addresses == null || addresses.length == 0) {
				...
				for (Address address : addresses) {
					for (SiteHost host : list) {
						...
						// 地址匹配则保留
						if (address.equals(host.getAddress())) {
							if (prime) {
								if (site.isPrime()) array.add(host);
							} else {
								array.add(host);
	 ...
	 * @return
	 */
	public Address[] rebuild(Space space, short columnId, Address[] hosts) {
		Logger.debug("DataPool.rebuild, space '%s', ip size %d", space, (hosts == null ? 0 : hosts.length));

		List<SiteHost> array = findHosts(space, hosts, true);
		ArrayList<Address> results = new ArrayList<Address>();
		for (SiteHost host : array) {
			DataClient client = fetch(host.getStreamHost());
			if (client != null) {
				try {
					boolean success = client.rebuild(space, columnId);
					if (success) {
						results.add( host.getAddress() ); 
					}
	...
	/**
	 * 通知匹配的DATA节点，从磁盘加载索引到内存
	 * 
	 * @param space
	 * @param hosts
	 * @return
	 */
	public Address[] loadIndex(Space space, Address[] hosts) {
	 ...
	 * @return
	 */
	public Address[] stopIndex(Space space, Address[] hosts) {
		...
		for (SiteHost host : array) {
			...
			if (client != null) {
				try {
					...
					if (success) {
						results.add( host.getAddress() ); // new IP(host.getIPValue()));
					}
				} catch (IOException exp) {
					Logger.error(exp);
				}
	 ...
	 * @return
	 */
	public Address[] loadChunk(Space space, Address[] hosts) {
	...
	/**
	 * 通知匹配的DATA节点，从内存释放数据块
	 * 
	 * @param space
	 * @param hosts
	 * @return
	 */
	public Address[] stopChunk(Space space, Address[] hosts) {
...
//	}
	...
	public long showChunkSize(String schema, int siteType, Address[] sites) throws VisitException {
		...
		try {
			for (Space space : mapSpace.keySet()) {
				if(space.getSchema().equalsIgnoreCase(schema)) {
					SiteSet set = mapSpace.get(space);
					if (siteType == Site.DATA_SITE) { // choose data-site
		...
		// connect data-site, get chunksize
		long chunksize = 0L;
		for (SiteHost host : array) {
			DataClient client = this.fetch(host.getStreamHost());
			if (client == null) continue;
			try {
				long size = client.showChunkSize(schema);
	...
	public long showChunkSize(String schema, String table, int siteType, Address[] sites) throws VisitException {
		...
		for (SiteHost host : array) {
			...
			if (client == null) continue;
			try {
				long size = client.showChunkSize(schema, table);
				if (size > 0L) chunksize += size;
			} catch (VisitException exp) {
				Logger.error(exp);
			} catch (Throwable exp) {
	...
	/**
	 * query "diffuse" naming and ip address
	 * @param addresses
	 * @return
	 */
	public String showTask(Address[] addresses) {
		...
		try {
			for(Naming naming : mapNaming.keySet()) {
				...
				if(addresses == null || addresses.length ==0) {
					...
					for(SiteHost host : set.list()) {
						if (host.getAddress().matchsIn(addresses)) {
							String s = String.format("%s %s\r\n", naming.toString(), host.getSpecifyAddress());
							buff.append(s);
						}
					}
		...
		try {
			...
			for(Space space : schema.keySet()) {
				...
				for (long chunkId : table.keys()) {
					ChunkAttribute sheet = table.find(chunkId);
			...
			// 通知所有CALL/WORK节点，更新全部DATA节点记录
			if (notify) {
				CallPool.getInstance().refreshDataSite();
				WorkPool.getInstance().refreshDataSite();
			}
...
//		}
		...
		for(SiteHost host: hosts) {
			SocketHost socket = host.getStreamHost();
	...
	public boolean deleteSpace(Space space) {
		...
		for (SiteHost host : sites) {
			...
			try {
				client.connect(host.getStreamHost());
				boolean success = client.deleteSpace(space);
				if (success) count++;
				client.exit();
			} catch (IOException exp) {
	...
	private boolean distribute(SiteHost host, PublishChunk chunk) {
		SocketHost address = host.getStreamHost();
