...
/**
 * 
 * HOME节点上，监管BUILD节点的资源配置池
 *
 */
public class BuildPool extends ControlPool {
	...
	/* chunk identity set */	
	private ChunkSet array = new ChunkSet();
...
//		for (int i = 0; targets != null && i < targets.length; i++) {
//			if (source.equals(targets)) return true;
//		}
//		return false;
//	}
	
	/**
	 * query "build" naming and ip address
	 * @param sites
	 * @return
	 */
	...
	private boolean match(SiteHost host, Address[] addresses) {
	 ...
	 * @return
	 */
	public Address[] buildTask(String name, Address[] hosts) {
		...
		// rpc to build site
		ArrayList<Address> store = new ArrayList<Address>();
		for(SiteHost host : array) {
			// choose ip address
			if (!match(host, hosts)) continue;
			// send "build task" command
			BuildClient client = fetch(host.getStreamHost());
			if(client == null) continue;
			try {
				boolean success = client.build(name);
				if (success) store.add( host.getAddress() ); 
			} catch (IOException exp) {
				Logger.error(exp);
			} catch (Throwable exp) {
				Logger.error(exp);
		...
		Address[] s = new Address[store.size()];
	...
	public boolean agree(long chunkid, SiteHost host) {
		...
		try {
			if (mapSite.containsKey(host)) {
				ArrayList<Address> array = new ArrayList<Address>( mapSite.size());
				for(SiteHost site : mapSite.keySet()) {
					array.add(site.getAddress());
				}
				java.util.Collections.sort(array);
				
				int remainder = (int) (chunkid % array.size());
				if(remainder < 0) remainder = Math.abs(remainder);
				Address address = array.get(remainder);
				allow = address.equals(host.getAddress());
...
//				allow = (ip == host.getIPValue());
	...
	public boolean accede(Space space, long chunkid, long length,
		...
		try {
			...
			ChunkStatus backup = set.find(chunkid);
			if (backup == null) {
				set.add(info);
				allow = true;
	...
	private void check() {
		...
		try {
			for (SiteHost host : mapTime.keySet()) {
				...
				if (now - time.longValue() >= getDeleteTimeout()) {
					dele.add(host);
				} else if (now - time.longValue() >= getRefreshTimeout()) {
					array.add(host);
				}
			}
		} catch (Throwable exp) {
