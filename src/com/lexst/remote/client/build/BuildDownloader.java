...
public class BuildDownloader {
	 ...
	 * @return
	 */
	private BuildClient apply(SocketHost address) {
	...
	public boolean execute(SiteHost host, Space space, long chunkId, String filename) {
		return this.execute(host.getStreamHost(), space, chunkId, filename);
	}
	...
	public boolean execute(SocketHost host, Space space, long chunkId, String filename) {
		BuildClient client = this.apply(host);
		if (client == null) {
			return false;
		}
		// execute download
	 ...
	 * @return
	 */
	public boolean execute(BuildClient client, Space space, long chunkId, String filename) {
		long breakpoint = 0L;
		// checksum and data
		...
		while(!success) {
			Logger.info("BuildDownloader.execute, from %s donwload '%s' - %x - %d to %s",
			...
			if (code != Response.ACCEPTED) {
				Logger.error("BuildDownloader.execute, cannot accpeted! code %d", code);
				return false;
			}
			...
			if (!space.equals(sp) || chunkId != cid || breakpoint != resume) {
				Logger.error("BuildDownloader.execute, not match! [%s - %s] [%x - %x] [%d - %d]",
			...
			try {
				while (breakpoint < length) {
					...
					if (sum != checksum) {
						Logger.error("BuildDownloader.execute, checksum error! [%x-%x]", sum, checksum);
						break;
					}
					// JNI write
			...
			Logger.debug("BuildDownloader.execute, download finished! breakpoint:%d, length:%d", breakpoint, length);
	...
	/**
	 * connect build site and download chunk
	 * @param client
	 * @param space
	 * @param chunkId
	 * @param breakpoint
	 * @return
	 */
	private Stream connect(BuildClient client, Space space, long chunkId, long breakpoint) {
		// send download command and receive it
		Stream reply = null;
		try {
