...
public class RemoteClient {
	...
	public void setRemote(InetAddress address, int port) {
		if (stream_client != null) {
			stream_client.setRemote(new SocketHost(SocketHost.TCP, address, port));
		} else {
			packet_client.setRemote(new SocketHost(SocketHost.UDP, address, port));
		}
	}

	public void setRemote(SocketHost host) {
		this.setRemote(host.getInetAddress(), host.getPort());
	}
...
//	}
	...
	public InetAddress getRemoteIP() {
		if (stream_client != null) {
			return stream_client.getRemote().getInetAddress();
		} else {
			return packet_client.getRemote().getInetAddress();
		}
	}
...
//	}
	...
	public boolean initSecure(RSAPublicKey key, String algo, byte[] pwd) throws IOException {
		...
		byte[] raws = SecureEncryptor.rsa(key, algo_bytes, 0, algo_bytes.length);
		...
		if(stream_client != null) {
			...
			// save serial data
			stream.setData(raws, 0, raws.length);
			// 3. send and receive
