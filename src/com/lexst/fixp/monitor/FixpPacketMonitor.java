...
public class FixpPacketMonitor extends FixpMonitor implements IPacketListener {
	 ...
	 * @param port
	 */
	public FixpPacketMonitor(InetAddress address, int port) {
		this();
		this.setLocal(address, port);
	}
...
//		local.setPort(host.getPort());
	...
	public void setLocal(InetAddress ip, int port) {
		local.setInetAddress(ip);
		local.setPort(port);
	}
	...
	private boolean bind() {
		...
		if(print) {
			com.lexst.log.client.Logger.info("FixpPacketMonitor.bind, bind to %s:%d", local.getAddress(), port);
		}
	...
	private boolean send(SocketHost remote, byte[] b, int off, int len) {
		SocketAddress address = new InetSocketAddress(remote.getInetAddress(), remote.getPort());
		boolean success = false;
		lock.lock();
		try {
		...
		while (!isInterrupted()) {
			try {
				...
				if (raw != null && raw.length > 0) {
					Cipher cipher = findCipher(remote);
					if (cipher != null) {
						raw = cipher.decrypt(raw, 0, raw.length);
						if(raw == null) {
							throw new IOException("decrypt failed!");
						}
						request.setData(raw, 0, raw.length);
					}
				}
