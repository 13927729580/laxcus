...
final class FixpPacketHelper extends VirtualThread {
	...
	private boolean matchAddress(Packet packet) {
		...
		return remote.getSpecifyAddress().equalsIgnoreCase(ip);
...
//			long ip = remote.getIPv4() & Long.MAX_VALUE;
	...
	private Cipher init_secure(byte[] raws) {
		if (security == null) {
			return null;
		} else if (security.getFamily() != Security.CIPHERTEXT_CHECK
				&& security.getFamily() != Security.DOUBLE_CHECK) {
			return null;
		...
		byte[] data = null;
		try {
			data = SecureDecryptor.rsa(security.getPrivateKey(), raws, 0, raws.length);
