...
final class StreamTask extends VirtualThread {
	...
	private boolean matchAddress(Stream stream) {
		...
		return remote.getSpecifyAddress().equalsIgnoreCase(address);
	}
...
//			long ip = remote.getIPv4() & Long.MAX_VALUE;
			if (!security.isLegalAddress( remote.getAddress() )) {
				return false;
			}
...
//			long ip = remote.getIPv4() & Long.MAX_VALUE;
	...
	private short init_secure(byte[] raws) throws IOException {
		...
		} else if (security.getFamily() != Security.CIPHERTEXT_CHECK && security.getFamily() != Security.DOUBLE_CHECK) {
			return Response.NOTACCEPTED;
		}

		byte[] data = SecureDecryptor.rsa(security.getPrivateKey(), raws, 0, raws.length);
...
//		request.read(dataIn, dataOut, false);
		...
		if (cipher == null) {
			...
			if (data != null && data.length > 0) {
				byte[] b = cipher.decrypt(data, 0, data.length);
				if(b == null) {
