/**
 * @email admin@lexst.com
 *
 */
package com.lexst.sql.index.section;

import java.math.*;
import java.util.*;
import java.util.regex.*;

import com.lexst.sql.column.*;
import com.lexst.util.range.DoubleRange;

/**
 * @author scott.liang
 *
 */
public class DoubleSector extends ColumnSector {

	private static final long serialVersionUID = 31262216930170254L;

	private final static String RANGE = "^\\s*([0-9a-fA-F]{1,16}),([0-9a-fA-F]{1,16})\\s*$";
	
	/* double range set */
	private Set<DoubleRange> ranges = new TreeSet<DoubleRange>();
	
	/**
	 * default
	 */
	public DoubleSector() {
		super();
	}

	public boolean add(DoubleRange range){
		 return ranges.add(range);
	}
	
	public boolean remove(DoubleRange range) {
		return ranges.remove(range);
	}
	
	public java.util.Collection<DoubleRange> list() {
		return this.ranges;
	}
	
	public void clear() {
		this.ranges.clear();
	}
	/*
	 * (non-Javadoc)
	 * @see com.lexst.sql.statement.order.ColumnSector#getTag()
	 */
	@Override
	public String getTag() {
		return "double_section";
	}
	
	/*
	 * (non-Javadoc)
	 * @see com.lexst.sql.statement.order.Section#indexOf(com.lexst.sql.column.Column)
	 */
	@Override
	public int indexOf(Column column) {
		if (column == null || column.getClass() != com.lexst.sql.column.Double.class) {
			throw new ClassCastException("this is not double");
		}

		return indexOf((com.lexst.sql.column.Double)column);
	}

	/**
	 * @param column
	 * @return
	 */
	public int indexOf(com.lexst.sql.column.Double column) {
		// 如果范围定义是错误
		if (ranges.isEmpty()) return -1;
		if (column.isNull()) return 0;

		double value = column.getValue();
		int index = 0;
		for (DoubleRange range : ranges) {
			if (value < range.getBegin() || range.inside(value)) return index;
			index++;
		}
		return index - 1;
	}

	/**
	 * 
	 * @param begin
	 * @param end
	 * @param elements
	 * @return
	 */
	public int splitting(double begin, double end, int elements) {
		BigDecimal min = new BigDecimal(String.valueOf(begin));
		BigDecimal max = new BigDecimal(String.valueOf(end));
		BigDecimal count = new BigDecimal(String.valueOf(elements));

		BigDecimal size = max.subtract(min).add(BigDecimal.ONE); // end - begin + 1
		BigDecimal gap = size.divide(count);

		int pos = ranges.size();
		BigDecimal seek = min;
		for (int i = 0; i < elements; i++) {
			BigDecimal last = seek.add(gap);
			if (last.compareTo(max) > 0 || i + 1 == elements) last = max;
			
			DoubleRange range = new DoubleRange(seek.doubleValue(), last.doubleValue());
			ranges.add(range);
			
			if (last.compareTo(max) >= 0) break; // last >= max
			seek = last;
		}
		
		return ranges.size() - pos;
	}
	
	/**
	 * 转换成long类型
	 * @param s
	 * @return
	 */
	protected long toLong(String s) {
		long value = 0L;
		int shift = 0;
		int end = s.length();
		while (end > 0) {
			int gap = (end - 2 >= 0 ? 2 : 1);
			String sub = s.substring(end - gap, end);
			long num = java.lang.Long.valueOf(sub, 16) & 0xFF;
			value |= (num << shift);
			shift += 8;
			end -= gap;
		}
		return value;
	}

	/*
	 * (non-Javadoc)
	 * @see com.lexst.sql.index.section.ColumnSector#split(java.lang.String)
	 */
	protected int split(String s) {
		int size = ranges.size();
		Pattern pattern = Pattern.compile(DoubleSector.RANGE);
		String[] elements = s.split(";");
		for (String element : elements) {
			Matcher matcher = pattern.matcher(element);
			if (!matcher.matches()) {
				throw new IllegalArgumentException("invalid long sector syntax! " + element);
			}
			
			double begin = java.lang.Double.longBitsToDouble(toLong(matcher.group(1)));
			double end = java.lang.Double.longBitsToDouble(toLong(matcher.group(2)));
			ranges.add(new DoubleRange(begin, end));
		}

		return ranges.size() - size;
	}

	/*
	 * (non-Javadoc)
	 * @see com.lexst.sql.index.section.ColumnSector#assemble()
	 */
	@Override
	protected String assemble() {
		StringBuilder buff = new StringBuilder();
		for (DoubleRange range : ranges) {
			if (buff.length() > 0) buff.append(';');
			long min = java.lang.Double.doubleToLongBits(range.getBegin());
			long max = java.lang.Double.doubleToLongBits(range.getEnd());
			buff.append(String.format("%X,%X", min, max));
		}
		
		return buff.toString();
	}

}